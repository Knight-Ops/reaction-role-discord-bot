                            ...
                            if let Some(guild_id) = guild_id {
                                ...
                                if let Some(user_id) = user_id {
                                    ...
                                    match emoji {
                                        ...
                                        // This variant occurs when you just absolutely need a custom emoji in your 
                                        // channel because all the possible Unicode emoji's are not enough.
                                        ReactionType::Custom {id, name, ..} => {
                                            let name = if let Some(name) = name {
                                                name
                                            } else {
                                                println!("Custom emoji name is None?");
                                                return
                                            };
                                            ...
                                            // comparing to see if the unicode matches, then spliting on `:`, trim the last
                                            // split, which we assume is our role name
                                            role_list.lines().filter(|line| line.contains(&name)).for_each(|line| {
                                                role = match line.split(":").last() {
                                                    Some(role_split) => role_split.trim(),
                                                    None => {
                                                        println!("Error while parsing the pinned message, couldn't find role within {}", line);
                                                        return
                                        ...
                                        _ => {
                                            println!("Unsupported ReactionType!");
                                        }
                                    }
                                }
                            }
